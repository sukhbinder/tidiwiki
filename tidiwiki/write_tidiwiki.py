...
def getdata(ll, tag0):
    try:
        ...
        data = dict(created=getNowtime(), tags=tag0, title=clean_title(title), text=text+"\n"+date)
...
def get_title_page(page, tag0=None, title=None):
    ...
    data=dict(created=getNowtime(), tags=tag, title=clean_title(title), text=text)
    return data
...
def main():
    parser = argparse.ArgumentParser("Convert PDF/Calibre highlights to Tidlers")
    parser.add_argument("fname", type=str, help="Journal.pdf")
    parser.add_argument("-o", "--out", type=str, default=None)
    parser.add_argument("-m", "--mark", action="store_true", help="If provided, Calibre markdown is assumed")
    ...
    if args.mark and args.tag is None:
        raise TypeError("Please enter tag name using '-t tag' with markdown option")
